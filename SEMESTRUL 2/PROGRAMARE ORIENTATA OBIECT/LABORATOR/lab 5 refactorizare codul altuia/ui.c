...
{
    printf("Adauga oferta : tip, model, producatorul, pret, id\n");
    char tip[30], destinatie[30], data_plecarii[30];
    int pret;
...
void printOfferte(BigList* list)
{
    for(int i = 0; i< list->lista->dimensiune;i++)
    {
        ...
        printf("Id: %d; Tip: %s; model: %s; producatorul: %s; Pret: %d\n",m->id,m->tip,m->model,m->producer,m->pret);
    }
}
...
{
    printf("alegeti sortarea : model, pret: ");
    char crit[20],t[20];
    scanf("%s", t);
    if (strcmp(t, "pret")==0)
    {
